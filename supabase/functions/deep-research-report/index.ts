...
`).join('\n\n')}`;
    ...
    if (vectorStoreError || !vectorStoreData?.vector_store_id) {
      throw new EdgeFunctionError(ERROR_CODES.DB_ERROR, 'Failed to get vector store ID', vectorStoreError);
    }

    // Try to add to vector store but make it optional - don't block report generation if it fails
    try {
      // Check if report is too large (OpenAI has size limits)
      const reportBytes = new TextEncoder().encode(report).length;
      const MAX_FILE_SIZE = 20 * 1024 * 1024; // 20MB - conservative limit
      
      if (reportBytes > MAX_FILE_SIZE) {
        console.log(`Report too large (${reportBytes} bytes). Chunking by markdown headers...`);
        
        // Split by markdown headers as natural boundaries
        const chunks = report.split(/(?=^#{1,6}\s)/m)
          .filter(chunk => chunk.trim().length > 0);
        
        console.log(`Split report into ${chunks.length} chunks`);
        
        // Upload each chunk with proper naming
        const fileIds = [];
        for (let i = 0; i < chunks.length; i++) {
          try {
            const chunkName = `deep_research_report_part${i+1}.md`;
            console.log(`Uploading chunk ${i+1}/${chunks.length}: ${chunkName}`);
            
            const file = await assistantClient.files.create({
              file: new File([chunks[i]], chunkName, { type: 'text/markdown' }),
              purpose: 'assistants'
            });
            
            fileIds.push(file.id);
            console.log(`Successfully uploaded chunk ${i+1} with ID: ${file.id}`);
          } catch (chunkError) {
        ...
        if (fileIds.length > 0) {
          ...
          console.log(`Adding ${fileIds.length} chunks to vector store...`);
          await assistantClient.vectorStores.fileBatches.createAndPoll(
            vectorStoreData.vector_store_id,
            { file_ids: fileIds }
          );
          console.log('Successfully added chunked report to vector store');
        } else {
          console.log('No chunks were successfully uploaded. Skipping vector store update.');
        }
      } else {
        // Create file in OpenAI and add to vector store
        console.log('Adding report to vector store...');
        ...
        console.log('Successfully added report to vector store');
      }
    } catch (vectorError) {
      // Log the error but continue with the rest of the process
      console.error('Failed to add report to vector store:', vectorError);
      console.log('Continuing without vector store update');
    }
